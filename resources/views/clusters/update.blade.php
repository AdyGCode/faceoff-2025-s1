    ...
    <div class="mx-auto max-w-7xl sm:px-2 lg:px-4">
      <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
        <div class="p-4 text-gray-900">
          <form method="POST" action="{{ route('clusters.update', $cluster->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mt-4">
              ...
              <x-text-input
                ...
                name="qualification"
                type="text"
                :value="old('qualification')?? $cluster->qualification"
                required
                autofocus
              />
            ...
            <div class="mt-4">
              ...
              <x-text-input
                ...
                name="qs_code"
                type="text"
                :value="old('qs_code')?? $cluster->qs_code"
                autofocus
              />
                ...
                @foreach($courses as $course)
                  <label class="flex items-center w-1/2 md:w-1/3 lg:w-1/4">
                    <input 
                      ...
                      name="course_id[]" 
                      value="{{ $course->id }}" 
                      {{ in_array($course->id, old('course_id', $cluster->courses->pluck('id')->toArray() ?? [])) ? 'checked' : '' }}
                      class="form-checkbox"
                    >
                ...
                @foreach($units as $unit)
                  <label class="flex items-center w-1/2 md:w-1/3 lg:w-1/4">
                    <input 
                      ...
                      name="unit_id[]" 
                      value="{{ $unit->id }}"
                      {{ in_array($unit->id, old('unit_id', $cluster->units->pluck('id')->toArray() ?? [])) ? 'checked' : '' }}
                      class="form-checkbox"
                    >
